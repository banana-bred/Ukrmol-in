...
use common_obj_gbl, only: nucleus_type, ecp_obj

   private

   public geometry_obj, symmetry_obj, determine_pg, Xlm_mgvn, is_centre_symmetric
...
contains
   ...
   subroutine read_geometry_obj(this,lunit,posit,pos_after_read,version)
      use mpi_gbl
      use const_gbl, only: data_file_obj_id_10, data_file_obj_id_11, line_len
      implicit none
      class(geometry_obj) :: this
      ...
      integer :: err, i, use_symmetry
         ...
         if (myrank .eq. master) then
            ...
            do i=1,this%no_nuc
               ...
               write(level2,'(a,i3,a3,f4.1,3f15.6)') 'Nucleus ', this%nucleus(i)%nuc, this%nucleus(i)%name, &
                                                                 this%nucleus(i)%charge, this%nucleus(i)%center
            enddo

            ! read flag as int, convert to logical (for portability; different compilers represent .TRUE. by different integers)
            read(lunit,err=10) use_symmetry
            this%use_symmetry = (use_symmetry /= 0)
            inquire(lunit,pos=pos_after_read)

            ! backwards compatibility with files generated prior to ECP implementation
            if (version /= data_file_obj_id_11 .and. version /= data_file_obj_id_10) then
               ! read the ECPs
               do i=1,this%no_nuc
                  call this%nucleus(i)%ecp%read_ecp(lunit,pos_after_read)
                  inquire(lunit,pos=pos_after_read)
               enddo
            else
               this%nucleus(:)%ecp = ecp_obj()  !use class defaults
            endif
         ...
         do i=1,this%no_nuc
            call this%nucleus(i)%ecp%mpi_ecp_bcast
         enddo
   ...
   subroutine write_geometry_obj(this,lunit,posit,pos_after_write)
      ...
      integer :: err, i
         ...
         if (myrank .eq. master) then
            ...
            ! write flag as int (for portability; different compilers represent .TRUE. by different integers)
            write(lunit,err=10) merge(1, 0, this%use_symmetry)
            inquire(lunit,pos=pos_after_write)

            ! write the ECPs at the end for backwards compatibility reasons
            do i=1,this%no_nuc
               call this%nucleus(i)%ecp%write_ecp(lunit,pos_after_write)
               inquire(lunit,pos=pos_after_write)
            enddo
         endif
...
!            if (cnt1 > 1 .or. cnt2 > 1) call xermsg('symmetry','check_symmetry_obj','Too many nuclei centered on the CMS.',13,1)
   ...
   !> \brief   Get (zero-based) irreducible representation of a real spherical harmonic
   !> \authors J Benda
   !> \date    2025
   !>
   !> We use the symmetry properties of the real spherical harmonics. Assuming that X, Y and Z are mirror operations
   !> by coordinate planes orthogonal to the given axes, i.e.,
   !>
   !> - X: phi -> pi - phi
   !> - Y: phi -> -phi
   !> - Z: theta -> pi - theta
   ...
   !> - Z X_{l,+|m|} = (-1)^(l-m) * X_{l,+|m|}
   !>
   !> Of these, only the specified symmetry elements are used. The relevant characters of the supported groups are the following
   !> (consistent with Molpro convention):
   !>
   !> - C1:  A
   !> - Ci:  Ag (XYZ+),    Au  (XYZ-)
   !> - Cs:  A' (X+),      A'' (X-)
   ...
   !>
   !> To maintain a predictable pattern in these tables, the symmetry operations have to be sorted by length (i.e. 'Z' first,
   !> then 'XY') and by alphabet ('X' first, then 'Y').
   !>
   integer function Xlm_mgvn(n, s, l, m) result(mgvn)

      use const_gbl, only: sym_op_nam_len

      character(len=sym_op_nam_len), intent(in) :: s(:)
      integer, intent(in) :: n, l, m
      integer :: W, X, Y, Z, i, j

      mgvn = 0

      ! characters for simple coordinate plane mirroring (operations X, Y, Z)
      X = (-1)**m * merge(+1, -1, m >= 0)
      Y = merge(+1, -1, m >= 0)
      Z = (-1)**(l - m)
      ...
      do i = 1, n
         ! combine plane mirror operations
         W = 1
         do j = 1, len_trim(s(i))
            select case (s(i)(j:j))
               case ('x', 'X'); W = W*X
         ...
         ! set the corresponding bit of 'mgvn'
         if (W == -1) mgvn = mgvn + 2**(i - 1)
      end do

   end function Xlm_mgvn


   !> \brief   Check if centre is symmetric w.r.t the symmetry operations
   !> \authors J Benda
   !> \date    2025
   !>
   !> Given that all symmetry operations in UKRmol+ are composed of axis inversions, the check needs to verify simply that
   !> whenever a specific axis inversion is included in any composed symmetry operation, the corresponding component of
   !> centre position is zero.
   !>
   logical function is_centre_symmetric(centre, no_sym_op, sym_op) result(ok)
      ...
      ok = .true.
      do i = 1, no_sym_op
         do j = 1, len_trim(sym_op(i))
            select case (sym_op(i)(j:j))
               case ('x', 'X'); if (centre(1) /= 0) ok = .false.
               case ('y', 'Y'); if (centre(2) /= 0) ok = .false.
               case ('z', 'Z'); if (centre(3) /= 0) ok = .false.
            end select
         end do
      end do

   end function is_centre_symmetric
