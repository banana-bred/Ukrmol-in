...
use utils_gbl
use precisn_gbl, only: cfp_dummy, f1mach
...
contains 
  ...
  !> For a B-spline with index ind which doesn't overlap with the quadrature grid the values in bspline_start_end_r are: bspline_start_end_r(1,ind) = 0, bspline_start_end_r(3,ind) = -1.
  !> The value in bspline_start_end_r(2,ind) is the end-point of the B-spline adjusted so that it is never larger than the point corresponding to the R-matrix radius.
  subroutine map_knots_to_grid(knots,order,last_bspline,r,rmat_radius,bspline_start_end_r)
     use precisn_gbl
     implicit none
     integer, intent(in) :: last_bspline, order
     real(kind=cfp), intent(in) :: knots(:), r(:), rmat_radius
     !OUTPUT:
     integer, allocatable :: bspline_start_end_r(:,:)

     integer :: i, err, ind, n_total_points, last_r_inner

        n_total_points = size(r)

        if (allocated(bspline_start_end_r)) deallocate(bspline_start_end_r)

        allocate(bspline_start_end_r(3,last_bspline),stat=err)
        ...
        do ind=1,last_bspline
           ...
           if (bspline_start_end_r(1,ind) .eq. 0) then !this B-spline doesn't overlap with the quadrature grid.
              bspline_start_end_r(3,ind) = -1
           else
              bspline_start_end_r(3,ind) = n_total_points
       ...
       !Adjust the end-points of the B-splines which reach beyond the R-matrix radius.
       last_r_inner = n_total_points
       do i=1,n_total_points
          if (r(i) .le. rmat_radius) last_r_inner = i
       enddo

       do i=1,last_bspline
          bspline_start_end_r(2,i) = min(last_r_inner,bspline_start_end_r(3,i))
       enddo
