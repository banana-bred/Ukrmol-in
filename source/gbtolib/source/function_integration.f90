...
module function_integration_gbl
 use general_quadrature_gbl, only: function_1d, function_1d_many
 use quadrature_module_gbl
 use precisn_gbl

 implicit none

 !> \f$ r^{l} \f$
 type, extends(function_1d) :: power_function
 ...
 !> \f$ r^{l}\exp[ar^{2}] \f$
 type, extends(function_1d) :: poly_exp_function
      real(kind=cfp) :: l !power
 ...
 type, extends(function_1d) :: bspl_prod_pow
 ...
 type, extends(function_1d) :: radial_function
 ...
 type, extends(function_1d) :: radial_function_buff
      integer :: li      !angular momentum of the B-spline orbital
      integer :: l1      !angular momentum of the spherical harmonic from the translation of the spherical harmonic of the GTO
      integer :: l       !angular momentum of the partial wave in the SCE of the GTO
      real(kind=cfp) :: alpha   !exponent of the GTO
 ...
 type, extends(function_1d) :: bessel_fn
 ...
 type, extends(function_1d_many) :: BB_legendre_integrand
      ...
      !> Multiplicative factor used in the evaluation. Typically you'll want it to be: fac = fourpi/(2*l+1.0_cfp)*norm1*norm2,
      !> where norm1, norm2 are the normalization factors for the two B-splines.
      ...
      real(kind=cfp), allocatable :: bcoef1(:), bcoef2(:)
      !> Array of knots.
      ...
      !todo add CGTO
 contains
      !> \memberof BB_legendre_integrand
      procedure :: wp_eval => wp_res
      !> \memberof BB_legendre integrand
      procedure :: ep_eval => ep_res
      !> \memberof BB_legendre_integrand
      procedure :: wp_eval_many => wp_res_many
      !> \memberof BB_legendre_integrand
      procedure :: ep_eval_many => ep_res_many
 end type

 private wp_res, ep_res
...
contains
 ...
 real(wp) function wp_radial_evaluation(data,x) result(r)
      use special_functions_gbl, only: cfp_besi
 ...
 real(ep1) function ep_radial_evaluation_buff(data,x) result(r)
      use special_functions_gbl, only: cfp_besi !_direct
      use utils_gbl, only: xermsg
      implicit none
      class(radial_function_buff) :: data
      real(ep1), intent(in) :: x
...
!         endif
 ...
 real(wp) function wp_bessel_eval(data,x) result(r)
      use special_functions_gbl, only: cfp_besj
      use utils_gbl, only: xermsg
      implicit none
 ...
 function wp_res(data,x)
      use bspline_base_gbl, only: bvalu
      implicit none
      class(BB_legendre_integrand) :: data
      real(wp), intent(in) :: x
      real(wp) :: wp_res
      real(cfp) :: x_conv
      ...
      integer :: i
         ...
         !todo add construct from grid_mod
         !call CGTO_amplitude instead of bvalu, ...
         !todo check the data%knots... if it covers the whole region
         !todo because eval_lambda calls this

         wp_res =        bvalu(data%knots,data%bcoef1,data%n,data%order,0,x_conv,data%inbv,data%work)
         wp_res = wp_res*bvalu(data%knots,data%bcoef2,data%n,data%order,0,x_conv,data%inbv,data%work)

         if (x .le. data%r1) then
            wp_res = wp_res*data%fac*(x/data%r1)**data%l
         else !x > data%r1
            wp_res = wp_res*data%fac*(data%r1/x)**(data%l+1)
         endif


 end function wp_res

 function wp_res_many(data,x,n)
      use bspline_base_gbl, only: bvalu
      implicit none
      class(BB_legendre_integrand) :: data
      integer, intent(in) :: n
      real(wp), intent(in) :: x(n)
      real(wp) :: wp_res_many(n)
      real(cfp) :: x_conv
      ...
      integer :: i
         ...
         do i=1,n
            ...
            x_conv = x(i)
            wp_res_many(i) =                bvalu(data%knots,data%bcoef1,data%n,data%order,0,x_conv,data%inbv,data%work)
            wp_res_many(i) = wp_res_many(i)*bvalu(data%knots,data%bcoef2,data%n,data%order,0,x_conv,data%inbv,data%work)

            if (x(i) .le. data%r1) then
               wp_res_many(i) = wp_res_many(i)*data%fac*(x(i)/data%r1)**data%l
            else !x > data%r1
               wp_res_many(i) = wp_res_many(i)*data%fac*(data%r1/x(i))**(data%l+1)
            endif

         enddo

 end function wp_res_many

 function ep_res(data,x)
      use bspline_base_gbl, only: bvalu
      implicit none
      class(BB_legendre_integrand) :: data
      real(ep1), intent(in) :: x
      ...
      integer :: i
         ...
         x_conv = x
         ep_res =        bvalu(data%knots,data%bcoef1,data%n,data%order,0,x_conv,data%inbv,data%work)
         ep_res = ep_res*bvalu(data%knots,data%bcoef2,data%n,data%order,0,x_conv,data%inbv,data%work)

         if (x .le. data%r1) then
            ep_res = ep_res*data%fac*(x/data%r1)**data%l
         else !x > data%r1
            ep_res = ep_res*data%fac*(data%r1/x)**(data%l+1)
         endif

 end function ep_res

 function ep_res_many(data,x,n)
      use bspline_base_gbl, only: bvalu
      implicit none
      class(BB_legendre_integrand) :: data
      integer, intent(in) :: n
      real(ep1), intent(in) :: x(n)
      real(ep1) :: ep_res_many(n)
      ...
      integer :: i
         ...
         do i=1,n
            ...
            x_conv = x(i)
            ep_res_many(i) =                bvalu(data%knots,data%bcoef1,data%n,data%order,0,x_conv,data%inbv,data%work)
            ep_res_many(i) = ep_res_many(i)*bvalu(data%knots,data%bcoef2,data%n,data%order,0,x_conv,data%inbv,data%work)

            if (x(i) .le. data%r1) then
               ep_res_many(i) = ep_res_many(i)*data%fac*(x(i)/data%r1)**data%l
            else !x > data%r1
               ep_res_many(i) = ep_res_many(i)*data%fac*(data%r1/x(i))**(data%l+1)
            endif

         enddo

 end function ep_res_many

end module function_integration_gbl
