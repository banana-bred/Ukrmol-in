...
module eri_sph_coord_gbl
 use precisn_gbl
 use gto_routines_gbl, only: boys_function_obj, index_1el, index_2el, eri_tail_shell, reorder_and_index_2el
 use coupling_obj_gbl, only: couplings_type
 use utils_gbl,        only: reserve_space
...
contains
   ...
   subroutine olap_kei_sph (lena,xa,ya,za,acnorm,anorms,la,aexps,acoefs,ind_a, &
      ...
      integer, intent(inout) :: int_index(:,:)
      logical, parameter :: calculate_indices = .false.
         ...
         if (la .ge. lb) then !la,lb
            ...
            call index_1el(la,lb,ind_a,ind_b,1,int_index,calculate_indices)
         else !lb,la
            ...
            call index_1el(lb,la,ind_b,ind_a,1,int_index,calculate_indices)
         endif
   ...
   subroutine ECP_atomic_integrals(nucleus, shell_A, shell_B, A, B, starting_index_A, starting_index_B, &
      ...
      type(nucleus_type), intent(in) :: nucleus
      integer, intent(in) :: A,B, starting_index_A, starting_index_B, ecp_column
      real(kind=cfp), intent(out) :: integrals(:,:)
      integer, intent(inout) :: int_index(:,:)

      integer :: i, j, lA, lB, mA, mB, jA, jB, m, terms
      real(kind=cfp) :: rad_ecp_int, g, cAB
      ...
      logical, parameter :: calculate_indices = .false.
         ...
         if (shell_A%l /= shell_B%l) return

         allocate(cA(shell_A%number_of_primitives),cB(shell_B%number_of_primitives))

         do jA=1,shell_A%number_of_primitives
            cA(jA) = sqrt(4*pi/(2*lA+1))*shell_A%norm*shell_A%contractions(jA)*shell_A%norms(jA)
         enddo

         do jB=1,shell_B%number_of_primitives
            cB(jB) = sqrt(4*pi/(2*lB+1))*shell_B%norm*shell_B%contractions(jB)*shell_B%norms(jB)
         enddo
         ...
         do jA=1,shell_A%number_of_primitives
            do jB=1,shell_B%number_of_primitives
               ...
               !V_loc
               do j=1,nucleus%ecp%nlmax
                  rad_ecp_int = rad_ecp_int + cAB*nucleus%ecp%cloc(j)&
                  &*atomic_ecp_GG_int(lA,lB,nucleus%ecp%mloc(j),shell_A%exponents(jA),shell_B%exponents(jB),&
                                     &nucleus%ecp%gloc(j))
               enddo !j

               !The semi-local term contributes only if there is a matching projector
               if (nucleus%ecp%have_semi_local_ecp .and. (lA <= nucleus%ecp%lmax-1)) then
                  do j=1,nucleus%ecp%nl(lA)
                     rad_ecp_int = rad_ecp_int + cAB*nucleus%ecp%c(j,lA)*&
                     &atomic_ecp_GG_int(lA,lB,nucleus%ecp%m(j,lA),shell_A%exponents(jA),shell_B%exponents(jB),&
                                       &nucleus%ecp%g(j,lA))
                  enddo !j
               endif
               ...
               if (nucleus%ecp%have_so_ecp .and. (lA > 0) .and. (lA <= nucleus%ecp%lmaxp)) then
                  ...
                  do j=1,nucleus%ecp%nlp(lA)
                     rad_ecp_int = rad_ecp_int + cAB*nucleus%ecp%cSO(j,lA)*&
                     &atomic_ecp_GG_int(lA,lB,nucleus%ecp%mSO(j,lA),shell_A%exponents(jA),shell_B%exponents(jB),&
                                       &nucleus%ecp%gSO(j,lA))
                  enddo !j
               endif
         ...
         do mB=-lB,lB
           do mA=-lA,lA
              i = i + 1
              if (mA == mB) then
                 integrals(i,ecp_column) = rad_ecp_int
              endif
           enddo
         enddo

   end subroutine ECP_atomic_integrals

   !> Computes the integral \int_{0}^{\infty} r^{lA+lB+m}*exp[-(a+b+g)*r^2] = 1/2 * alp^{-1/2-n/2}*\Gamma[(1+n)/2], where n = lA+lB+m, alp = a+b+g
   function atomic_ecp_GG_int(lA,lB,m,a,b,g)
      implicit none
      integer, intent(in) :: lA,lB,m
      real(kind=cfp), intent(in) :: a,b,g
      real(kind=cfp) :: atomic_ecp_GG_int
      ...
      real(kind=cfp) :: alp
         ...
         if (n > -1) then
            atomic_ecp_GG_int = 0.5_cfp * alp **(-0.5_cfp*(1+n)) * gamma(0.5_cfp*(1+n))
         else
            stop "n < -1 not implemented"
         endif

   end function atomic_ecp_GG_int
   ...
   subroutine allocate_space(la,lb,lc,ld, lena,lenb,lenc,lend)
      ...
      integer :: space_cpl_AB, space_G, n_l21m21, sph_a, sph_b, space_F_X, n_contr_pair, space_sum_AB, &
      ...
      real(kind=cfp) :: f
         ...
         if (l_1 > max_l_1 .or. l_2 > max_l_2 .or. cont_len > max_len) then
            if (allocated(G_A)) deallocate(G_A,G_B,F_X,sum_AB,contr_AB,sum_CD,l_ijkl_L_coefficients,cf,SH_ml,SH_l)
         ...
         allocate(G_A(space_G),G_B(space_G),F_X(space_F_X),sum_AB(space_sum_AB),contr_AB(space_contr_AB),sum_CD(space_sum_CD), &
                  l_ijkl_L_coefficients(space_l_ijkl_L_coefficients),cf(space_cf),SH_ml(2,(sum_l+1)**2),SH_l(sum_l+1),stat=err)
   ...
   subroutine eri_shell_sph (lena,xa,ya,za,anorms,la,aexps,acoefs, &
      ...
      use phys_const_gbl, only: twopi, fourpi
      use const_gbl, only: imax_wp,boys_f_grid_step_wp,taylor_k_wp, imax_ep,boys_f_grid_step_ep,taylor_k_ep,mmax
      implicit none
...
!
   ...
   subroutine calculate_ABCD_coefficients(la_p_lb,lc_p_ld,n_ij,n_kl,l_ijkl_L_coefficients,solid_harmonics_abcd,cpl_ABCD)
       ...
       real(kind=cfp), allocatable, intent(inout) :: cpl_ABCD(:)

       integer :: l21, m21, l43, m43, big_l, l,m, ij, kl, base_solid_harmonics, base, ind, l_min, l_max, n_ijkl, &
                  terms_to_skip, base_l, ind_sum, ijkl, n_l, base_cf, n_l_total, i, maxn1, maxn2, maxn
       ...
       integer :: dAB, dBA
          ...
          ! compute memory requirements of this subroutine, and realloc the storage when needed
          ! (maxn1 and maxn2 are simply the maximal values of `ind` and `i`, respectively, attained in this call)
          maxn1 = lm_total*(la_p_lb + lc_p_ld) + n_ij*n_l21m21 + n_l21m21*n_ijkl*(n_l43m43 - 1) + n_l21m21*n_ij*(n_kl - 1)
          maxn2 = lm_total*(la_p_lb + lc_p_ld) + n_ij*n_l43m43 + n_l21m21*n_ijkl*(n_l21m21 - 1) + n_l21m21*n_ij*(n_kl - 1)
          maxn = max(maxn1, maxn2)
          call reserve_space(cpl_ABCD, maxn)
