...
module const_gbl
   ...
   !> Kinetic energy+Nuclear attraction (3), Nuclear attraction (4), ECP integrals (5), property integrals (6), bbb integrals (7).
   integer, parameter :: number_of_types_el_ints = 7
   ...
   character(len=*), parameter :: ecp_ints = 'Effective Core Potential integrals'
   character(len=*), parameter :: bbb_ints = 'Overlap integrals for product of three basis functions'
   ...
   character(line_len), parameter :: data_file_obj_id_12 = 'DATA FILE version 1.2'
   character(line_len), parameter :: data_file_obj_id = data_file_obj_id_12  ! this is the current version
   ...
   !> Length of the elementary G-L quadrature when calculating the CGTO-only ECP integrals numerically.
   real(kind=cfp), parameter :: delta_r_ecp = 0.01_cfp
   ...
   !> Threshold used to test that two nuclei have the same center.
   real(kind=cfp), parameter :: thrs_center_same = 10**(-(precision(cfp_dummy)-5.0_cfp)) !10E-10_cfp for double precision
   ...
   !> Threshold for switch to asymptotic method for calculation of the u-integral in cgto_pw_expansions_mod/Boys_projected_on_X_lm (double precision).
   real(kind=cfp), parameter :: exp_arg_asymptotic_wp = 45.0_cfp
   !> Threshold for switch to asymptotic method for calculation of the u-integral in cgto_pw_expansions_mod/Boys_projected_on_X_lm (quad precision).
   real(kind=cfp), parameter :: exp_arg_asymptotic_ep = 2*exp_arg_asymptotic_wp
   ...
   !> Maximum number of terms in the definition of the radial ECP potential.
   integer, parameter :: nlmax_ecp = 20

   !> Maximum L in the ECP potential
   integer, parameter :: lmax_ecp = 10
