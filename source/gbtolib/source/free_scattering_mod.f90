...
module free_scattering_gbl
   use precisn_gbl
   use mpi_gbl, only: mpi_xermsg, mpi_mod_barrier
...
contains
   ...
   subroutine free_scattering(mo_integral_storage,molecular_orbital_basis,a,min_energy,max_energy,nE)
      use const_gbl, only: level1, level2, level3, kinetic_ints, one_p_sym_ints, ecp_ints
      ...
      integer :: tot_chan, col_kei, col_ecp
      logical, parameter :: normalize_to_a = .true., test_ecp = .false. !.true.

         write(level3,'(/,"--------->free_scattering_mod:free_scattering")')
         ...
         if (test_ecp) col_ecp = mo_integrals%find_column_matching_name(ecp_ints)
         ...
         do sym=1,molecular_orbital_basis%no_irr
            ...
            write(level1,'(/,"Symmetry: ",i1)') sym

            n = molecular_orbital_basis%get_number_of_orbitals(sym)

            if (n .eq. 0) then
               write(level1,'("No orbitals in this symmetry: skipping")')
               cycle
            endif
            ...
            do i=sym_start,sym_end
               do j=sym_start,i
                  ind(1:1) = molecular_orbital_basis%integral_index(kinetic_ints,reshape(source=(/j,i/),shape=(/2,1/)),&
                                                                    options%two_p_continuum)
                  ham(j-sym_start+1,i-sym_start+1) = mo_integrals%a(ind(1),col_kei)

                  if (test_ecp) then
                     ind(1:1) = molecular_orbital_basis%integral_index(ecp_ints,reshape(source=(/j,i/),shape=(/2,1/)),&
                                                                    options%two_p_continuum)
                     ham(j-sym_start+1,i-sym_start+1) = ham(j-sym_start+1,i-sym_start+1) + mo_integrals%a(ind(1),col_ecp)
                  endif
            ...
            write(level2,'(/,"Eigenvalues of the Kinetic energy matrix:")')
            do i=1,n
               write(level2,'(i0,e25.15)') i,e(i)
               if (e(i) < 0.0_cfp) then
                  call mpi_xermsg ('free_scattering_mod', 'free_scattering', &
            ...
            write(level1,'(/,"Partial waves (m,l) in the current symmetry:")')
            n_chan = 0
            do i=1,tot_chan
               if (chan_lm_all(3,i) .eq. sym) then 
                  n_chan = n_chan + 1
                  write(level1,'(2i5)') chan_lm_all(1:2,i)
               endif
            enddo

            write(level1,'(/,"Number of channels in the current symmetry: ",i0)') n_chan
            ...
            do i=1,nE+1
               ...
               write(level1,'(e25.15,e25.15)') energy*to_ev,sum(atan(eigphase))
            enddo
         ...
         write(level3,'(/,"<---------done:free_scattering_mod:free_scattering")')
   ...
   subroutine calc_kmat_e(kmatrix,a0,energy,n_chan,l_list,rmatrix)
      ...
      use blas_lapack_gbl, only: blasint, trsm
      implicit none
...
end module free_scattering_gbl
