...
!
module bspline_grid_gbl
   use precisn_gbl
   use utils_gbl, only: xermsg
   use mpi_gbl
   use const_gbl, only: level2, level3
   ...
   !> A, B, C, order, l,m and ind, no_bps. This object can then be passed to bspl_function%init, which checks the parameters
   ...
   contains
      ...
      !> This procedure sets up the grid of radial B-splines given the values: A, B, C order, no_bps. The variables initialized
      ...
      !> the user can set-up the grid parameters by hand to whatever grid needed. The points A and B delimit the radial range
      !> of the B-spline grid and the point C should be a point somewhere in this range. The B-spline grid is then set-up in such
      !> a way that the point C is forced to be one of the knot points for the B-spline grid. The intention was to interpret
      !> the point C as the R-matrix radius so that the B-spline grid can extend beyond it. This can be useful for codes based
      !> on methods other than the R-matrix. If fixing a gridpoint at C is not required, input C outside of the range (A,B)
      procedure :: init_grid
      ...
      !> Returns an array mapping the product of a pair of B-splines to a unique pair of B-splines. If the pair of B-splines
      !> does not overlap the corresponding value in pairs_to_unique_pairs is zero.
      procedure :: get_unique_pairs
      !> Returns the index of the last B-spline which starts before a given radial point R.
      !> If R .le. 0.0_cfp then the last inner B-spline is equal to the total number of B-splines.
      procedure :: get_last_inner_bspline
...
!   subroutine init_grid_master(this,A,B,order,no_bsplines)
...
!         enddo
!         
!         !uniform spacing of knots ('min' ensures that the knot value does not overflow this%B even by a tiny amount)
!         do i = this%order+1, this%no_knots-this%order
!            this%knots(i) = min((this%A*(this%no_bps - 1 - i + this%order) + this%B*(i - this%order)) / (this%no_bps - 1), this%B)
!         enddo
!         !todo temp
...
!
!   end subroutine init_grid_master

   subroutine init_grid(this,A,B,C,order,no_bsplines)
      use bspline_base_gbl, only: bvalu
      implicit none
      class(bspline_grid_obj) :: this
      real(kind=cfp), intent(in) :: A,B,C
      integer, intent(in) :: order,no_bsplines

      integer :: err, i, j
      real(kind=cfp) :: cfp_dummy, val, old
         ...
         allocate(this%knots(1:this%no_knots),this%bcoef(1:this%n),this%work(1:3*this%order),stat=err)
         if (err /= 0) call xermsg ('bto_function', 'init_grid', 'Memory allocation failed', err, 1)
         ...
         !Replace the knot closest to the point C with point C: this ensures
         !that if C represents the R-matrix radius somewhere between (A;B) it will always be one of the knot points.
         if ((C > A) .and. (C < B)) then
            old = B + 1.0_cfp
            do i=this%order,this%no_knots-(this%order-1)
               val = abs(this%knots(i)-C)
               if (val .le. old) then
                  j = i
                  old = val
               endif
            enddo
            write(stdout,'(10X," Knot",i4,1X,e25.15," has been replaced with point: ",e25.15)') j, this%knots(j), C
            this%knots(j) = C !Replace the knot closes to point C with the point C
         endif

         !Find the last B-spline starting before point C:
         this%ind_last_before_C = this%get_last_inner_bspline(this%C)
   ...
   function normalize(this,ind)
      ...
      use bspline_base_gbl, only: bvalu
      use general_quadrature_gbl, only: gl_expand_A_B, w_10, x_10, n_10
      use quadrature_module_gbl, only: cfp_bsqad
      implicit none
      class(bspline_grid_obj) :: this
      integer, intent(in) :: ind
      real(kind=cfp) :: normalize

      integer, parameter :: n_gauss = 2*n_10+1
      integer :: i, j, err, der
      real(kind=cfp) :: quad, r1, r2, B_r1r2(n_gauss), r_r1r2(n_gauss), w_r1r2(n_gauss)
      type(power_function) :: f_poly
         ...
         der = 0 !no derivative of the B-spline

         this%inbv = 1
         quad = 0.0_cfp

         if (2*n_gauss-1 >= 2*(this%order-1)) then
            !use the n-point Gauss quadrature for B-splines of order (polynom. degree this%order-1) up to: 2*n-1 >= 2*(order-1)
            !the reason for 2*(order-1) is that we're integrating the square of the B-spline which doubles the polynomial order

            !integrate over the distinct piecewise sections spanning the B-spline
            do i = ind, ind+this%order -1
               r1 = this%knots(i)
               r2 = this%knots(i+1)
               if (r2 > r1) then
                  call gl_expand_A_B(x_10,w_10,n_10,r_r1r2,w_r1r2,r1,r2)
                  do j = 1, n_gauss
                     B_r1r2(j) = bvalu(this%knots,this%bcoef,this%n,this%order,der,r_r1r2(j),this%inbv,this%work)
                     B_r1r2(j) = B_r1r2(j) * B_r1r2(j)
                  enddo !j
                  quad = quad + sum(B_r1r2*w_r1r2)
               endif
            enddo !i
            ...
            call cfp_bsqad(f_poly, this%knots, this%bcoef, this%n, this%order, der, r1, r2, this%tol, quad, err, this%work)
            if (err /= 1) then
   ...
   subroutine read(this,lunit,posit,pos_after_rw,version)
      use const_gbl, only: line_len, data_file_obj_id_12
      implicit none
      class(bspline_grid_obj) :: this
      ...
      real(kind=cfp) :: bcast(1:3)
         ...
         if (myrank .eq. master) then
            if (version < data_file_obj_id_12) then
               read(lunit,pos=posit,err=10) this%A, this%B
            else
               read(lunit,pos=posit,err=10) this%A, this%B, this%C
            end if
 ...
 10      call xermsg ('bspline_grid_obj', 'read', 'Error reading the B-spline grid data from the file and position given.', 1, 1)
   ...
   subroutine write(this,lunit,posit,pos_after_rw)
      ...
      integer :: err
         ...
         if (myrank .eq. master) then
            write(lunit,pos=posit,err=10) this%A, this%B, this%C
 ...
 10      call xermsg ('bspline_grid_obj', 'write', 'Error writing the B-spline grid data into the file and position given.', 1, 1)
   ...
   subroutine print(this)
      ...
      integer :: err
         ...
         write(level3,'("Parameters of the B-spline grid:")')
         write(level3,'("A, B, C: ",3e20.10)') this%A, this%B, this%C
         write(level3,'("Order of the radial B-splines: ",i0)') this%order
         write(level3,'("Number of break-points: ",i0)') this%no_bps
         write(level3,'("Number of B-splines in the basis: ",i0)') this%n
   ...
   subroutine print_grid(this)
      ...
      integer :: err, i
         ...
         write(level3,'(/,"Array of knots: ")')
         do i=1,size(this%knots)
            write(level3,'(i5,e25.15)') i,this%knots(i)
         enddo !i
   ...
   subroutine read_BTO(bspline_grid,l,bspline_index,number_of_functions,norm,non_zero_at_boundary,lunit,posit,pos_after_rw,version)
      ...
      integer :: start
         ...
         start = pos_after_rw
         call bspline_grid%read(lunit,start,pos_after_rw,version)
 ...
 10      call xermsg ('bspline_grid_obj', 'write_BTO', 'Error writing the BTO data from the file and position given.', 1, 1)
   ...
   subroutine print_BTO(bspline_grid,l,bspline_index,number_of_functions,norm,non_zero_at_boundary)
      ...
      real(kind=cfp) :: r1, r2
         ...
         write(level3,'("L: ",i0)') l
         write(level3,'("Index of the radial B-spline, total number of radial B-splines &
                      &generated by the present grid: ",i0,1x,i0)') bspline_index, bspline_grid%n
         write(level3,'("Normalization factor: ",e20.10)') norm
         write(level3,'("Is non-zero at the boundary: ",l1)') non_zero_at_boundary

         call bspline_grid%bspline_range(bspline_index,r1,r2)
         write(level3,'("Radial range of the B-spline: ",2e20.10)') r1, r2
   ...
   subroutine get_unique_pairs(this,pairs_to_unique_pairs,n_unique_pairs)
      implicit none
      class(bspline_grid_obj) :: this
      integer, allocatable :: pairs_to_unique_pairs(:,:)
      integer, intent(out) :: n_unique_pairs

      integer :: ind1,ind2,err
      real(kind=cfp) :: s1,s2,e1,e2,s_point,e_point

      if (.not. allocated(this%knots)) then
         call xermsg ('bspline_grid_obj', 'get_unique_pairs', 'The B-spline grid has not been initialized.', 1, 1)
      endif

      !First count how many unique non-zero products of B-splines we have
      n_unique_pairs = 0
      do ind1=1,this%n
         call this%bspline_range(ind1,s1,e1)
         do ind2=1,ind1
            call this%bspline_range(ind2,s2,e2)
            s_point = max(s1,s2)
            e_point = min(e1,e2)
            if (s_point < e_point) then
                n_unique_pairs = n_unique_pairs + 1
            endif
         enddo !ind2
      enddo !ind1

      if (allocated(pairs_to_unique_pairs)) deallocate(pairs_to_unique_pairs)
      allocate(pairs_to_unique_pairs(this%n,this%n),stat=err)
      if (err /= 0) call xermsg ('bspline_grid_obj', 'get_unique_pairs','Memory allocation failed.', err, 1)

       n_unique_pairs = 0
      pairs_to_unique_pairs = 0
      do ind1=1,this%n
         call this%bspline_range(ind1,s1,e1)
         do ind2=1,ind1
            call this%bspline_range(ind2,s2,e2)
            ...
            if (s_point < e_point) then
                n_unique_pairs = n_unique_pairs + 1
                pairs_to_unique_pairs(ind1,ind2) = n_unique_pairs
                pairs_to_unique_pairs(ind2,ind1) = n_unique_pairs
            endif
         enddo !ind2
      enddo !ind1

   end subroutine get_unique_pairs

   function get_last_inner_bspline(this,R)
      implicit none
      class(bspline_grid_obj) :: this
      real(kind=cfp), intent(in) :: R

      integer :: get_last_inner_bspline
      integer :: ind
      real(kind=cfp) :: r1,r2

         if (.not. allocated(this%knots)) then
            call xermsg ('bspline_grid_obj', 'get_last_inner_bspline', 'The B-spline grid has not been initialized.', 1, 1)
         endif

         get_last_inner_bspline = this%n
         do ind=1,this%n
            call this%bspline_range(ind,r1,r2)
            if (r1 < R) get_last_inner_bspline = ind
         enddo

   end function get_last_inner_bspline



end module bspline_grid_gbl
