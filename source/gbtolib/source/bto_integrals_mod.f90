...
module bto_integrals_gbl
   ...
   use symmetry_gbl
!UNCOMMENT TO DEBUG
!   use cgto_pw_expansions_gbl, only: dbg_cgto, init_dbg
   implicit none
   ...
   real(kind=cfp), allocatable :: r_points_leg(:), weights_leg(:), B_vals_leg(:,:,:), radial_prod(:)

   !> Radial integrals over ECPs.
   real(kind=cfp), allocatable :: rad_ecp_int(:,:,:), rad_ecp_loc(:,:,:)
   logical :: have_ECP_only_on_CMS

   !> Maximum L in the angular momentum projector of the ECP, if used.
   integer :: lmax_ecp = -1

   !> Maximum L in the BTO basis.
   integer :: inp_max_l_bto = -1

   !> Maps start and end of each B-spline to the quadrature points of the radial grid.
   integer, allocatable :: bspline_start_end(:,:), bspline_start_end_leg(:,:)
   ...
   real(kind=cfp), allocatable :: prop_on_grid(:,:), leg_on_grid(:,:), re_sph_harm_center(:,:)
   ...
   real(kind=cfp) :: dipole_damp_factor_saved = 0.0_cfp

   !> Used to compute the triple overlap integrals BBB
   type(BTO_shell_data_obj), allocatable :: BTO_shells_saved(:)

   !> Used to store the triple overlap integrals BBB
   ...
   integer :: no_cgto_shells_saved = 0
...
!      use gto_routines_gbl, only: compare_print_1el_ints
      use symmetry_gbl
      ...
      integer, intent(in) :: starting_index_A, starting_index_B, olap_column,kei_column,nai_column,prop_column,&
                             one_elham_column,max_property_l,ecp_column,bbb_column
      ...
      real(kind=cfp) :: bloch_el, radial_olap, radial_kei_1, radial_kei_2, kei, fac, radial_bbb
      integer :: m1, m2, column, n_ang_prop, lm, terms, A_ind_leg, B_ind_leg, shell_ind
      real(kind=cfp) :: radial_prop, s, rad_ecp
      logical :: zero
...
!     character(len=4) :: tag="BB"
         ...
         if (one_elham_column > 0 .and. (kei_column == 0 .or. nai_column == 0)) then
            call xermsg ('bto_integrals_mod', 'BB_shell_integrals', &
         ...
         if (B_ind .le. A_ind) then
            ...
            if (ecp_column > 0) integrals(1:terms,ecp_column) = 0.0_cfp
            ...
            if (bbb_column > 0) integrals(1:terms,bbb_column:) = 0.0_cfp  ! TODO: explicit upper bound
            return
         endif
         ...
         if (bbb_column > 0) then
            ...
            !Loop over the BTO shells from <B_A|B|B_B>.
            do i = 1, size(BTO_shells_saved)

               shell_ind = BTO_shells_saved(i) % bspline_index
               l = BTO_shells_saved(i) % l
   
               !Radial B*B/r*B integral:
               radial_bbb = sum(B_vals_leg(1,A_ind_leg:B_ind_leg,shell_A_ind) &
                              * B_vals_leg(1,A_ind_leg:B_ind_leg,shell_B_ind) &
                              * B_vals_leg(1,A_ind_leg:B_ind_leg,shell_ind)/r_points_leg(A_ind_leg:B_ind_leg) &
                              * weights_leg(A_ind_leg:B_ind_leg))
   
               column = (bbb_column-1) + shell_starting_indices_saved(no_cgto_shells_saved + i) - 1
               ...
               do m = -l,l
                  ...
                  do m2=-shell_B%l,shell_B%l
                     do m1=-shell_A%l,shell_A%l
                        ind = ind + 1
                        integrals(ind,column) = radial_bbb*cpl%rgaunt(l,shell_A%l,shell_B%l,m,m1,m2)
                     enddo !m1
                  enddo !m2
               enddo !m
...
!            enddo !i
            ...
            !$OMP PARALLEL DO COLLAPSE(2) SCHEDULE(DYNAMIC) DEFAULT(NONE) PRIVATE(m1,m2,l,m,lm,fac,s,ind) &
            !$OMP& SHARED(shell_A,shell_B,cpl,A_ind_leg,B_ind_leg,leg_on_grid,radial_prod,integrals,nai_column)
            do m2=-shell_B%l,shell_B%l
               do m1=-shell_A%l,shell_A%l
                  ...
                  ind = (shell_B%l + m2)*(2*shell_A%l + 1) + shell_A%l + m1 + 1
...
!        BB integrals involving B-splines and ECPs away from CMS are not implemented.

         !Selection rule for the atomic ECP integrals.
         zero = (shell_A%l /= shell_B%l)

         if (ecp_column > 0 .and. have_ECP_only_on_CMS .and. .not. zero) then
            integrals(1:terms,ecp_column) = 0.0_cfp

            !The local term (saved in L=-1 component) always contributes
            ind = -1
            rad_ecp = rad_ecp_int(shell_A_ind,shell_B_ind,ind)

            !Add the semi-local term if non-zero
            if (shell_A%l <= lmax_ecp) rad_ecp = rad_ecp + rad_ecp_int(shell_A_ind,shell_B_ind,shell_A%l)

            !TODO deal with the SO term?

            i = 0
            do m2=-shell_A%l,shell_A%l
              do m1=-shell_A%l,shell_A%l
                 i = i + 1
                 if (m1 == m2) then
                    integrals(i,ecp_column) = rad_ecp
                 endif
              enddo
            enddo
         endif
   ...
   subroutine BB_initialize(bspline_grid,max_bspline_l,max_prop_l,dipole_damp_factor,symmetry_data,rmat_radius,&
                            BTO_shells,no_CGTO_shells,shell_starting_indices)
      use symmetry_gbl
      use sort_gbl, only: sort_float
      use general_quadrature_gbl, only: n_10,w_10,x_10, n_7,w_7,x_7
      use bspline_base_gbl
      use phys_const_gbl, only: fourpi
      use special_functions_gbl, only: cfp_resh
      use const_gbl, only: epsabs, level1
      implicit none
      type(bspline_grid_obj), intent(inout) :: bspline_grid
      integer, optional, intent(in) :: max_prop_l,max_bspline_l
      real(kind=cfp), intent(in) :: dipole_damp_factor, rmat_radius
      type(symmetry_obj), intent(in) :: symmetry_data
      type(BTO_shell_data_obj), intent(in) :: BTO_shells(:)
      integer, intent(in) :: no_CGTO_shells
      integer, intent(in) :: shell_starting_indices(:)

      integer :: i, j, k, l, m, n_total_points, err, ind, lm, n_total_points_leg, n_centers, ind_i, ind_j, i1, i2
      real(kind=cfp) :: bto_norm, fac, R, r1, r2, val, rad_BB_w 
      real(kind=cfp), allocatable :: xlm(:,:), list(:,:)
      real(kind=cfp), allocatable :: local_ecp(:), semi_local_ecp(:,:), so_ecp(:,:)
...
!         call init_dbg(bspline_grid)
         ...
         if (err /= 0) then
            call xermsg ('bto_integrals_mod', 'BB_initialize', &
                'Memory allocation 1 failed.', err, 1)
         endif

         !Number of CGTO shells in the basis = number of shells preceding BTOs in shell_starting_indices_saved
         no_cgto_shells_saved = no_cgto_shells
         ...
         call map_knots_to_grid(bspline_grid%knots,&
                                bspline_grid%order,&
                                bspline_grid%n,&
                                r_points,&
                                rmat_radius,&
...
!                                         &*(1+l-2*dbg_cgto(ind)%exponents(1)*r2**2)
         ...
         do l=0,max_prop_l
            ...
            if (dipole_damp_factor_saved /= 0.0_cfp .and. l == 1) then
               do i=1,n_total_points
                  prop_on_grid(i,l) = prop_on_grid(i,l) * exp(-dipole_damp_factor_saved*r_points(i))
               enddo !i
            endif
         enddo !l
         ...
         if (max_bspline_l > 0) then
            do k=1,n_centers
               ...
               do l=0,2*max_bspline_l
                  do m=-l,l
                     lm = l*l+l+m+1
                     re_sph_harm_center(k,lm) = -(symmetry_data%nucleus(k)%charge-symmetry_data%nucleus(k)%ecp%n_core)*xlm(m,l)
                  enddo !m
               enddo !l
            enddo !k
         else !l .eq. 0
            do k=1,n_centers
               re_sph_harm_center(k,1) = -(symmetry_data%nucleus(k)%charge-symmetry_data%nucleus(k)%ecp%n_core)/sqrt(fourpi)
            enddo
         endif
         ...
         !Precalculate the radial ECP integrals for the atom on the CMS, if present
         have_ECP_only_on_CMS = .false.
         do j=1,n_centers
            associate(nucleus => symmetry_data%nucleus(j))
            R = nucleus%r_CMS()
            if (nucleus%ecp%have_ecp .and. R < 10e-10_cfp) then

               have_ECP_only_on_CMS = .true.

               call nucleus%ecp%eval_local_ecp(r_points_leg,local_ecp)
               if (nucleus%ecp%have_semi_local_ecp) call nucleus%ecp%eval_semi_local_ecp(r_points_leg,semi_local_ecp)
               if (nucleus%ecp%have_so_ecp) then
                  call nucleus%ecp%eval_so_ecp(r_points_leg,so_ecp)
                  stop "not implemented yet"
               endif

               !Dump the radial ECPs to a file
               !do i=1,size(r_points_leg)
               !   write(12,'(100(1X,e25.15e5))') r_points_leg(i),local_ecp(i),semi_local_ecp(i,0:nucleus%ecp%lmax-1)
               !enddo

               lmax_ecp = maxval([nucleus%ecp%lmax-1,nucleus%ecp%lmaxp,1])
               if (allocated(rad_ecp_int)) deallocate(rad_ecp_int)
               allocate(rad_ecp_int(bspline_grid%n,bspline_grid%n,-1:lmax_ecp))
               rad_ecp_int = 0.0_cfp

               do ind_j = 1,bspline_grid%n
                  do ind_i = 1,ind_j
                     i1 = max(bspline_start_end_leg(1,ind_i),bspline_start_end_leg(1,ind_j))
                     i2 = min(bspline_start_end_leg(2,ind_i),bspline_start_end_leg(2,ind_j))
                     do i = i1, i2
            
                        r = r_points_leg(i)
      
                        rad_BB_w = B_vals_leg(1,i,ind_i)*B_vals_leg(1,i,ind_j)*weights_leg(i)
      
                        !The local term always contributes. Save it as L = -1 component
                        l = -1
                        rad_ecp_int(ind_i,ind_j,l) = rad_ecp_int(ind_i,ind_j,l) + rad_BB_w*local_ecp(i)
            
                        !The semi-local term contributes only if there is a matching projector: save it in a separate array entry
                        if (nucleus%ecp%have_semi_local_ecp) then
                           do l = 0, nucleus%ecp%lmax-1
                              rad_ecp_int(ind_i,ind_j,l) = rad_ecp_int(ind_i,ind_j,l) + rad_BB_w*semi_local_ecp(i,l)
                           enddo
                        endif
            
                        !The SO term contributes only if there is a matching projector
      !                  if (nucleus%ecp%have_so_ecp .and. (lA > 0) .and. (lA <= nucleus%ecp%lmaxp)) then
      !                     !TODO include the action of the orbital angular momentum op.
      !                     !TODO also this integral probably needs to be kept separate
      !                     do l = 1, nucleus%ecp%lmaxp
      !                        rad_ecp_int_so(ind_i,ind_j,l) = rad_ecp_int_so(ind_i,ind_j,l) + rad_BB_w*so_ecp(i,l)
      !                     enddo
      !                  endif
            
                     enddo !i

                     !symmetry in B-spline indices
                     do l = -1, lmax_ecp
                        rad_ecp_int(ind_j,ind_i,l) = rad_ecp_int(ind_i,ind_j,l)
                     enddo

                  enddo !ind_i
               enddo !ind_j

            elseif (nucleus%ecp%have_ecp .and. R >= 10e-10_cfp) then
         ...
         if (have_ECP_only_on_CMS) then
            write(level1,'(/,10X,"Detected a single ECP potential on a CMS atom.")')
         endif
