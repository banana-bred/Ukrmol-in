...
module cgto_integrals_gbl
   ...
   use gto_routines_gbl
   use utils_gbl
   use cgto_pw_expansions_gbl, only: CGTO_shell_pair_pw_expansion_obj, init_CGTO_pw_expansions_mod
   use gxg_integrals_gbl, only: init_GXG_integrals, init_G_ECP_G_integrals
   ...
   logical :: tails_1el, tails_2el, keep_ab_cd_order_saved, two_p_continuum_saved, have_bto_basis
   real(kind=cfp) :: rmat_radius, dipole_damp_factor_saved = 0.0_cfp
...
contains
   ...
   subroutine GG_initialize(CGTO_shells, BTO_shells, a, dipole_damp_factor, delta_r1, bbb_column, ecp_column, symmetry_data, &
                            shell_starting_indices)
      use symmetry_gbl
      use const_gbl, only: delta_r_ecp
      implicit none
      type(CGTO_shell_data_obj), intent(in) :: CGTO_shells(:)
      type(BTO_shell_data_obj), optional, intent(in) :: BTO_shells(:)
      real(kind=cfp), intent(in) :: a, dipole_damp_factor, delta_r1
      type(symmetry_obj), intent(in) :: symmetry_data
      integer, intent(in) :: bbb_column, ecp_column, shell_starting_indices(:)

      integer :: i, inp_max_l_cgto, max_l_pw
         ...
         if (dipole_damp_factor_saved /= 0.0_cfp) then
            if (rmat_radius <= 0.0_cfp) then
               call xermsg ('cgto_integrals', 'GG_initialize', &
                   'Damped dipolar integrals can by calculated only when the rmat_radius has &
                    &been explicitly set.', 1, 1)
            endif

            call GG_pair_pw % eval_regular_grid(0.0_cfp, rmat_radius, delta_r1)
         ...
         if (bbb_column > 0) then
            if (present(BTO_shells)) then
               have_bto_basis = .true.
               call init_GXG_integrals(CGTO_shells, BTO_shells, shell_starting_indices, symmetry_data, a, delta_r1)
            else
               have_bto_basis = .false.
               call init_GXG_integrals(CGTO_shells=CGTO_shells, shell_starting_indices=shell_starting_indices, &
                                      &symmetry_data=symmetry_data, a=a, delta_r1=delta_r1)
            endif
         endif

         if (ecp_column > 0) then
            call init_G_ECP_G_integrals(CGTO_shells, shell_starting_indices, symmetry_data, a, delta_r_ecp)
         endif

         if (size(shell_starting_indices) < size(CGTO_shells)) then
            call xermsg ('cgto_integrals', 'GG_initialize', &
                         'Incompatible sizes of shell_starting_indices and CGTO_shells.', 2, 1)
         endif
   ...
   !> \warning It is important to realize that the order of the generated integrals, i.e. the different (M_A,M_A) combinations, are prescribed by
   !> the first routine which generates int_index: typically the olap_kei_sph routine. ALL SUBSEQUENT INTEGRAL ROUTINES MUST OBEY THE SAME ORDER.
   !> TODO Generate int_index only once at the beginning of GG_shell_integrals and make it an INPUT array for all other routines which should then merely check
   !> that they generate the integrals in the correct order!
   subroutine GG_shell_integrals(shell_A,shell_B,A,B,starting_index_A,starting_index_B,&
                                     ...
                                     &olap_column,kei_column,prop_column,nai_column,ecp_column,bbb_column,one_elham_column,&
                                     &int_index,integrals)
      use cgto_hgp_gbl, only: sph_olap_kei, sph_nari, sph_mult_mom, index_1el
      use eri_sph_coord_gbl, only: olap_kei_sph
      use gxg_integrals_gbl, only: GGG_shell_integrals, ECP_shell_integrals, GBG_shell_integrals
      use gto_routines_gbl, only: olap_kei_tail, nari_tail, prop_cms_tail
      use symmetry_gbl
      implicit none
      ...
      integer, intent(in) :: A,B, starting_index_A, starting_index_B, olap_column,kei_column,nai_column,prop_column,&
                             ecp_column,bbb_column,one_elham_column,max_property_l
      ...
      real(kind=cfp) :: olap_tail,kei_tail_ab,bloch_ab,kei_tail_ba,bloch_ba, Zeff
      type(CGTO_shell_data_obj), pointer :: shell_Ap, shell_Bp
      logical, parameter :: calculate_indices = .true.
...
!------- GENERATE int_index: THE LIST OF PAIRS OF BASIS FUNCTION INDICES THAT CORRESPOND TO THE INTEGRALS SAVED IN integrals ARRAY.
!        THIS ORDER OF SAVING THE INTEGRALS MUST BE OBEYED IN ALL ROUTINES THAT CALCULATE THE 1-ELECTRON INTEGRALS.
         if (shell_A % l >= shell_B % l) then !shell_A % l,shell_B % l
            call index_1el(shell_A % l,shell_B % l,starting_index_A,starting_index_B,1,int_index,calculate_indices)
         else !shell_B % l,shell_A % l
            call index_1el(shell_B % l,shell_A % l,starting_index_B,starting_index_A,1,int_index,calculate_indices)
         endif
...
!------- Calculate integrals over all space:
         ...
         if (nai_column > 0) then !nuclear attraction integrals
            ...
            call sph_nari (shell_A % number_of_primitives, &
               ...
               !Accumulate the results into the nari_nuc array holding the final results and multiply the contributions by the effective nuclear charge.
               !The effective nuclear charge is obtained as the full nuclear charge minus the possible screening by the core electrons replaced by the ECPs.
               Zeff = symmetry_data%nucleus(i)%charge-symmetry_data%nucleus(i)%ecp%n_core
               integrals(1:terms,nai_column) = integrals(1:terms,nai_column) - Zeff*nari(1:terms)
            enddo
         ...
         if (prop_column > 0) then
            ...
            !Replace the standard dipolar integrals with those including the exponential damping factors
            if (dipole_damp_factor_saved /= 0.0_cfp) then

               call GG_pair_pw % eval_damped_dipole_integrals(shell_A, shell_B, A, B, starting_index_A, starting_index_B, &
                                 dipole_damp_factor_saved, prop_column, integrals, int_index)

            endif !if (dipole_damp_factor_saved /= 0.0_cfp)
         endif

         !ECP integrals: currently only the atomic ones
         if (ecp_column > 0) then

            integrals(1:terms,ecp_column) = 0.0_cfp

            if (shell_A % l >= shell_B % l) then
               call ECP_shell_integrals(shell_A, shell_B, A, B, &
                                         starting_index_A, starting_index_B, ecp_column, integrals, int_index)
            else
               call ECP_shell_integrals(shell_B, shell_A, B, A, &
                                         starting_index_B, starting_index_A, ecp_column, integrals, int_index)
            endif

         endif

         !Overlap integrals over three CGTOs, <G_A|G|G_B> and <G_A|B|G_B>. G_A,G_B are shell_A and shell_B while G/B are all CGTOs/BTOs in the basis.
         if (bbb_column > 0) then
            !We have to make sure the integrals on output are ordered so that int_index is unchanged
            if (shell_A%l >= shell_B%l) then
               call GGG_shell_integrals(shell_B, shell_A, B, A, starting_index_B, starting_index_A,bbb_column,int_index, integrals)
               if (have_bto_basis) then
                  call GBG_shell_integrals(shell_B, shell_A, B, A, starting_index_B, starting_index_A,bbb_column,int_index, &
                                           integrals)
               endif
            else
               call GGG_shell_integrals(shell_A, shell_B, A, B, starting_index_A, starting_index_B,bbb_column,int_index, integrals)
               if (have_bto_basis) then
                  call GBG_shell_integrals(shell_A, shell_B, A, B, starting_index_A, starting_index_B,bbb_column,int_index, &
                                           integrals)
               endif
            endif
         endif
         ...
         if (do_tails_for_this_pair) then
            ...
            if (nai_column > 0) then
               ...
               do i=1,symmetry_data%no_nuc
                  ...
                  Zeff = symmetry_data%nucleus(i)%charge-symmetry_data%nucleus(i)%ecp%n_core
                  na_tail(1:terms) = na_tail(1:terms) - Zeff*nari(1:terms)
               enddo
            ...
            if (prop_column > 0) then
               ...
               if (dipole_damp_factor_saved /= 0.0_cfp) then
                  ! delete the tail integrals for l = 1 since the damped dipolar integrals have been calculated explicitly over the R-matrix sphere
                  cnt = terms
                  prop_tail(cnt + 1: cnt + terms*3) = 0.0_cfp
               endif
   ...
   subroutine GGGG_initialize(CGTO_shells,shell_starting_indices,tol,a,keep_ab_cd_order,two_p_continuum,indexing_method_inp)
      use const_gbl
      use cgto_hgp_gbl, only: sph_mult_mom
      implicit none
      ...
      logical, parameter :: calculate_indices = .true.
         ...
         if (a > 0.0_cfp) then
            if (.not.(cgto_continuum)) then
               tails_2el = .false.
               write(level2,'("a > 0 but the basis contains no CGTO continuum functions so no CGTO tails have to be subtracted.")')
            elseif (cgto_continuum .and. number_of_target_shells > 0) then !there are some functions in the basis that represent the bound electrons
               if (.not.(two_p_continuum)) then
                  write(level2,'("Evaluation of integrals of the type [continuum,continuum|continuum,continuum] &
                                 &and [continuum,continuum|continuum,target] will be skipped.")')
                  write(level2,'("Tail integrals for ONE ELECTRON IN THE CONTINUUM and a = ",e25.15," a.u. &
                                 &will be calculated and subtracted.")') a
                  tails_2el = .true.
               else
                  write(level2,'("Tail integrals for TWO ELECTRONS IN THE CONTINUUM and a = ",e25.15," a.u. &
                                 &will be calculated and subtracted.")') a
                  tails_2el = .true.
               endif
               write(level2,'("Number of shells of target functions: ",i0)') number_of_target_shells
               write(level2,'("ASSUMPTION: all non-CMS functions fit completely inside the R-matrix sphere.")')
            elseif (cgto_continuum .and. number_of_target_shells .eq. 0) then !the basis contains only continuum GTOs
               if (.not.(two_p_continuum)) then
                  ...
                  write(level2,'("The GTO basis set does not contain any functions representing the bound electrons. &
            ...
            write(level2,'("R-matrix radius is not > 0.0_cfp; no tail integrals will be calculated.")')
            tails_2el = .false.
         endif

         if (two_p_continuum .and. tails_2el) then
            call xermsg ('cgto_integrals', 'GGGG_initialize', &
            'Tails for 2p in the continuum have been requested but they are not implemented. &
            &The 2p tail integrals will not be subtracted so we assume all continuum functions fit within the R-matrix sphere. &
            &See comments in GGGG_shell_integrals.',1, 0)
         endif
         ...
         if (tails_2el) then
            ...
            do i=1,number_of_target_shells
               do j=1,i
                  ...
                  if (CGTO_shells(i) % l >= CGTO_shells(j) % l) then !CGTO_shells(i) % l,CGTO_shells(j) % l
                     call index_1el(CGTO_shells(i) % l,CGTO_shells(j) % l,shell_starting_indices(i),shell_starting_indices(j),1,&
                                    mom_index,calculate_indices)
                  else !CGTO_shells(j) % l,CGTO_shells(i) % l
                     call index_1el(CGTO_shells(j) % l,CGTO_shells(i) % l,shell_starting_indices(j),shell_starting_indices(i),1,&
                                    mom_index,calculate_indices)
                  endif
   ...
   subroutine GGGG_shell_integrals(shell_A,shell_B,shell_C,shell_D,A,B,C,D,&
      ...
      use eri_sph_coord_gbl, only: eri_sph
      use gto_routines_gbl, only: eri_tail
      implicit none
         ...
         if (tails_2el) then
            ...
            elseif (.not.(is_continuum(A)) .and. .not.(is_continuum(B)) .and. (is_continuum(C) .and. is_continuum(D))) then !TT|CC
               ...
               if (count(cnt) > 2) then
                  ...
                  !todo Temporary switch disabling 2p tail integrals.
                  do_tails_for_this_quartet = .false.
   ...
   subroutine GGGG_final
      use cgto_hgp_gbl, only: cgto_hgp_final
      use eri_sph_coord_gbl, only: eri_sph_coord_final
      implicit none
