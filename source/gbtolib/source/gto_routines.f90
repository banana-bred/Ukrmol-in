...
use utils_gbl, only: xermsg
  ...
  !todo interface for master and towards_next_major_release versions
  interface reorder_and_index_2el
     module procedure reorder_and_index_2el_master, reorder_and_index_2el_towards
  end interface
...
contains
  ...
  function check_real_array_size(a,d)
   ...
   !> The logical variable 'calculate_indices' is used to determine how to interpret the int_index array: if set to .true. then int_index is treated as an input array whose values are compared 
   !> to the indexing scheme implemented here. This is used in throughout the code to check that all integrals are generated in the expected order.
   subroutine index_1el(la,lb,ind_a,ind_b,np,int_index,calculate_indices)
      implicit none
      ...
      logical, intent(in) :: calculate_indices

      integer :: a,b,i,p,sph_shell_a,i1,i2

      i = 0
      sph_shell_a = 2*la+1

      if (calculate_indices) then

         do p=1,np !this is the loop over the passive indices, i.e. possible repetition of the indexing sequence.
      ...
      else !check that the values from int_index coincide with those calculated here
         ...
         do p=1,np !this is the loop over the passive indices, i.e. possible repetition of the indexing sequence.
            do b=ind_b,ind_b+2*lb
               do a=ind_a,ind_a+2*la !the a shell index changes quickest, then b
                  ...
                  if (i1 /= int_index(1,i) .or. i2 /= int_index(2,i)) then
                     call xermsg ('gto_routines', 'index_1el', 'Inconsistency in the supplied int_index array.', 1, 1)
                  endif
               enddo
            enddo
         enddo
   ...
   subroutine reorder_and_index_2el_master(la,lb,lc,ld,ind_a,ind_b,ind_c,ind_d,column,ind,ints)
      implicit none
   ...
   end subroutine reorder_and_index_2el_master

   !> WARNING: THERE IS A MISTAKE IN THIS VERSION, PROBABLY IN THE symmetrize_batch SUBROUTINE
   !> Reorder the integrals ints in columns a,b,c,d so that ap.ge.bp,cp.ge.dp,ap.ge.cp. The input values are the angular momenta and the starting indices in columns 1,2,3,4 of the ints and ind arrays.
   !> On output the indices in ind(1:4,i) are not ordered from the largest to the smallest as the original ones (a.ge.b,c.ge.d,a.ge.c) but that's OK since they're not needed to save the integrals
   !> in the MPI mode when this routine is used.
   !> The routine also outputs the value n_unique which is the number of symmetry-unique integrals for which the reordering was done. If the array ind is given on input the indices corresponding
   !> to each integral are output too. The indices are not required when computing BBBB and BGBB integrals.
   subroutine reorder_and_index_2el_towards(la,lb,lc,ld,ind_a,ind_b,ind_c,ind_d,column,offset,ints,n_unique,ind)
      implicit none
      integer, intent(in) :: la,lb,lc,ld,ind_a,ind_b,ind_c,ind_d,column,offset
      !intent(out):
      integer, allocatable, optional :: ind(:,:)
      real(kind=cfp), allocatable :: ints(:,:)
      integer, intent(out) :: n_unique

      integer :: ind_ordered, ind_orig(4), i, n(4), n_map(3), map(4), i1,i2,i3,i4, a,b,c,d, err, n_ordered(4), batch_type
      logical :: ordered, do_indexing

         ind_orig(1:4) = (/ind_a,ind_b,ind_c,ind_d/)
         n(1:4) = (/2*la+1,2*lb+1,2*lc+1,2*ld+1/)

         do_indexing = present(ind)

         !todo quit if n_ints == 1

         !Find mapping of the four columns between the original order and the order where the columns are ordered accoding to the decreasing starting index.
         call find_mapping(ind_orig,n,n_map,map)

         n_unique = product(n)

         if (allocated(ints_tmp)) then
            if (size(ints_tmp) < n_unique) deallocate (ints_tmp)
         end if
         if (.not. allocated(ints_tmp)) then
            allocate(ints_tmp(n_unique),stat=err)
            if (err /= 0) call xermsg ('gto_routines', 'reorder_and_index_2el', 'Memory allocation 2 has failed.', err, 1)
         endif

         batch_type = identify_batch_type(ind_orig)

         !For ABCD type check if the required order and the present order are the same. If they are then there is nothing to do.
         if (batch_type .eq. 0) then
            ordered = .true.
            do i=1,4
               if (map(i) .ne. i) ordered = .false.
            enddo
         ...
         !the largest starting index, 2nd column corresponds to the shell with
         !the 2nd largest starting index, etc.
         ints_tmp(1:n_unique) = ints(offset+1:offset+n_unique,column)

         !todo since the ABCD is the most prevalent case we should be running it by default and only afterwards check if this was indeed ABCD.
         if (batch_type .eq. 0) then !the most general case ABCD: 4D matrix; this is the case which dominates by far, cases 1,2 are the next ones in frequency and the other ones form a negligible fraction.
               ...
               do c=1,n(3)
                  do b=1,n(2)
                     do a=1,n(1)
                        ...
                        i4 = ind_orig(map(4))
                        ind_ordered = i1 + n_map(1)*(i2-1) + n_map(2)*(i3-1) + n_map(3)*(i4-1)
                        if (do_indexing) ind(1:4,ind_ordered) = (/i1,i2,i3,i4/)
                        ints(offset+ind_ordered,column) = ints_tmp(i)
                     enddo !a
                  enddo !b
               enddo !c
            enddo !d
            ...
            n_ordered(3) = n(map(3))
            n_ordered(4) = n(map(4))
            call symmetrize_batch(ints_tmp,ints,column,offset,ind,map,n_map,n_ordered,n,batch_type,do_indexing,n_unique)

         endif

   end subroutine reorder_and_index_2el_towards

   !> On input ind_orig must come from a call to find_mapping.
   pure function identify_batch_type(ind_orig)
      implicit none
      integer, intent(in) :: ind_orig(4)
      integer :: identify_batch_type

      logical :: AB_is_AA,CD_is_CC,A_is_C,B_is_D,AABD_or_ABCC,AACC_or_AAAA

         AB_is_AA = (ind_orig(1) .eq. ind_orig(2))
         CD_is_CC = (ind_orig(3) .eq. ind_orig(4))
         A_is_C = (ind_orig(1) .eq. ind_orig(3))
         B_is_D = (ind_orig(2) .eq. ind_orig(4))

         identify_batch_type = -1

         AABD_or_ABCC = AB_is_AA .neqv. CD_is_CC

         if (AABD_or_ABCC) then !Exactly one of the AB, CD pairs of shells are the same
            if (AB_is_AA) then !AABD or AAAD
               identify_batch_type = 1
            else !CD_is_CC => ABCC or ACCC
               identify_batch_type = 2
            endif
         else !either both pairs AB and CD are identical or both pairs are different: we have (AACC or AAAA) or (ABCD or ABAB)
            AACC_or_AAAA = (AB_is_AA .and. CD_is_CC)
            if (AACC_or_AAAA) then
               if (A_is_C) then !AAAA
                  identify_batch_type = 3
               else !AACC
                  identify_batch_type = 4
               endif
            else
               if (A_is_C .and. B_is_D) then !ABAB
                  identify_batch_type = 5
               else !ABCD
                  identify_batch_type = 0
               endif
            endif
         endif

         !if (identify_batch_type .eq. -1) stop "error in batch identification"

   end function identify_batch_type

   pure subroutine symmetrize_batch(ints_tmp,ints,column,offset,ind,map,n_map,n_ordered,n,batch_type,do_indexing,n_unique)
      implicit none
      integer, intent(out) :: ind(:,:)
      real(kind=cfp), allocatable, intent(in) :: ints_tmp(:)
      real(kind=cfp), intent(out) :: ints(:,:)
      integer, intent(in) :: column, n(4), batch_type, map(4), n_map(3), n_ordered(4), offset
      logical, intent(in) :: do_indexing
      integer, intent(out) :: n_unique

      integer :: i,i1,i2,i3,i4,a,b,c,d,ind_ordered,s1,s2,t1,t2,t3,t4,ind_orig(4)

         select case (batch_type)
         case (0) !ABCD
            n_unique = product(n)
         case (1) !AABD
            s1 = n_ordered(1)*(n_ordered(1)+1)/2
            s2 = s1*n_ordered(3)
            n_unique = s2*n_ordered(4)
         case (2) !ABCC
            s2 = n_ordered(3)*(n_ordered(3)+1)/2
            s1 = n_ordered(1)*n_ordered(2)
            n_unique = s1*s2
         case (3) !AAAA
            s1 = n_ordered(1)*(n_ordered(1)+1)/2
            n_unique = s1*(s1+1)/2
         ...
         !It would be better to unroll the select case but perhaps the compiler is doing it anyway with optimization?
         i = 0
         do d=1,n(4)
            do c=1,n(3)
               do b=1,n(2)
                  do a=1,n(1)
                     ...
                     case (1) !AABD
                        t1 = max(i1,i2)
                        t2 = min(i1,i2)
                        ind_ordered = ipair(t1)+t2 + s1*(i3-1) + s2*(i4-1)
                     case (2) !ABCC
                        t1 = max(i3,i4)
                        t2 = min(i3,i4)
                        ind_ordered = i1 + n_map(1)*(i2-1) + n_map(2)*(ipair(t1)+t2-1)
                     case (3) !AAAA
                        t1 = max(i1,i2)
                        t2 = min(i1,i2)
                        t3 = max(i3,i4)
                        ...
                        t2 = ipair(t3)+t4
                        if (t1 < t2) then
                           ind_ordered = ipair(t2)+t1
                        else
                           ind_ordered = ipair(t1)+t2
                        endif
                     case(4) !AACC
                        t1 = max(i1,i2)
                        t2 = min(i1,i2)
                        ...
                        t1 = ipair(t1)+t2
                        t2 = ipair(t3)+t4
                        ind_ordered = t1 + s1*(t2-1)
                     case(5) !ABAB
                        t1 = i1 + n_ordered(1)*(i2-1)
                        t2 = i3 + n_ordered(3)*(i4-1)
                        s1 = max(t1,t2)
                        s2 = min(t1,t2)
                        if (s1 < s2) then
                           ind_ordered = ipair(s2)+s1
                        else
                           ind_ordered = ipair(s1)+s2
                        endif
                     end select
                     if (do_indexing) ind(1:4,ind_ordered) = (/i1,i2,i3,i4/)
                     ints(offset+ind_ordered,column) = ints_tmp(i)
   ...
   end subroutine symmetrize_batch
   ...
   function check_cgto_data(number_of_primitives,l,exponents,contractions,norms,number_of_functions)
      ...
      integer :: i,j
         ...
         if (number_of_primitives .le. 0) then
            check_cgto_data = 2
            return
         endif
